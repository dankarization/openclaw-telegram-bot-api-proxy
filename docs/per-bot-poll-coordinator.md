...
Every `getUpdates` request for one bot ID now enters a FIFO lane before the
proxy starts its buffered routing cycle. Different bot IDs use independent
lanes and may run concurrently.
...
The protected cycle includes:

1. request-body read and parsing;
2. local health check;
3. local long poll and every retry;
4. legacy native↔virtual bridge mutation;
5. stale local update filtering and internal `ack-dropped`;
6. cloud pending probe and opt-in rescue;
7. cloud fallback and response rewriting;
8. downstream response completion or terminal error.
